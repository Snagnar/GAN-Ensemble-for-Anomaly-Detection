...
from dataloader.datasets import AnnotatedRoadImageDataset, get_cifar_anomaly_dataset, MVTecDataset, PanoramaDataset
...
def load_data(opt):
    ...
    elif opt.dataset == "roadimages":

        train_ds = AnnotatedRoadImageDataset(opt.dataroot, train=True)
        valid_ds = AnnotatedRoadImageDataset(opt.dataroot, train=False)
        if opt.isize == -1:
            opt.isize = train_ds.isize
